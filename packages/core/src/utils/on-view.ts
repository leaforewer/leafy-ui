...
export function initOnView(options: OnViewOptions = {}) {
  ...
  observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        element.dispatchEvent(new CustomEvent('viewenter', {
          bubbles: true,
          detail: { element }
        }));
        ...
        if (!config.once) {
          ...
          element.dispatchEvent(new CustomEvent('viewexit', {
            bubbles: true,
            detail: { element }
          }));
        }
  ...
  elements.forEach(element => {
    ...
    const customThreshold = computedStyle.getPropertyValue('--on-view-threshold').trim();
    const customMargin = computedStyle.getPropertyValue('--on-view-margin').trim();
    ...
    if (customThreshold || customMargin) {
      const elementObserver = new IntersectionObserver((entries) => {
        ...
        threshold: customThreshold ? parseFloat(customThreshold) : config.threshold,
